...
**Trained brains are currently inert** (added 2026-08-03). All four personality `.onnx` files in `Assets/Agents/{Standard,Matt,Nick,Kim}_v01/` declare `obs_0 [batch, 66]` + `obs_1 [batch, 36]` (model API version 3 = MLAgents2_0, memory_size=0). The current `Agent_Soccer` only registers the 18-float VectorSensor (becomes `obs_1` after the RayPerception sensor is added back), so `obs_0` is "Unknown tensorProxy" and `SentisPolicy.DecideAction` throws every FixedUpdate. The four `Reward_*.asset` profiles have had `brainModel` cleared, so all agents run on `Agent_HeuristicBot` in `SCN_Exhibition` until the .onnx files are regenerated. **Re-training is blocked by**: (1) `.venv` has never been built (`setup-training-env.ps1` is unwritten territory), (2) ML-Agents 4.1.0's `RayPerceptionSensor.OutputSize() = (DetectableTags + 2) * Rays` (hit + normalized distance + tag one-hots) is structurally different from the mlagents 1.2.0.dev0 trainer used to export these `.onnx` (which had `(DetectableTags + 1) * Rays` — no normalized distance), so even with `Sensor_Vision` reattached the brains would interpret the observation differently and produce random actions. Realistic unblock: run `setup-training-env.ps1` first, then retrain STANDARD/MATT/KIM/NICK from scratch with the current sensor + observation contract before re-attaching brains.
...
**Open items:** `.venv` does not exist on this machine — `setup-training-env.ps1` is written but has never been run, so training/eval are blocked. KIM/MATT/NICK now have brainModels wired through `Agent_MatchLoader.Apply` (commit eab3b43) — they play their trained brains in `SCN_Exhibition` matches when selected from the menu. The `ai-game-developer` connector is unauthorized. Active-ragdoll articulation is an accepted open deviation (`docs/rules-exemptions.md`). Stamina wear-and-tear has no recovery path (documented trade-off, `docs/rules-exemptions.md` §4).
