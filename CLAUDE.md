...
**Brain contract** (enforced in `Agent_Soccer.Awake`, scene can't drift): behavior name from `brainName` (default `STANDARD`), **27 vector obs** (14 self/ball/goal + 4 teammate + 8 opponent = 2 nearest slots × rel-position/velocity + 1 time-remaining, all zero-padded when empty) × **2 stacked**, **4 continuous actions** (forward / lateral / turn / boost), DecisionRequester period 8. Total model inputs **108** = 54 ray + 54 vector. Changing any of this obsoletes every trained `.onnx`.
