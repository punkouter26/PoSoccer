...
| `soccer_v3_00` | ~15.7M | 18% | 18% |
| `soccer_p3_botcurric_00` | — | 19% | 12% |
| `soccer_p4_vsbot_00` | 7.0M | 24% | 14% |
| `soccer_p5_paced_00` (in the STANDARD slot) | 30.0M | **24%** | 18% |

The baseline is symmetric, so the harness is fair and the brains really are **worse than the rule-based bot**: parity would be ~42.5%, and the best policy sits at 24% while the bot takes 58%. Bar 80%/≤10% badly unmet.

**Compute has stopped buying wins.** `soccer_p5_paced_00` ran 4.3× the steps of `p4` (30.0M vs 7.0M), reached the top `bot_strength` lesson (0.8), and returned the *same* 24% with 4 points more stalemate. Its training reward never stabilized — the last checkpoints oscillate between −1.07 and +0.52, finishing at −0.045, i.e. the curriculum advanced on a noisy threshold rather than on mastery. Five runs across 5M→30M steps and two sensor geometries all land in a 15–24% band. Treat "train it longer" as a disproven lever.
...
Next levers, in the order the evidence supports: (1) add opponent terms to the vector observations, or handicap `nearestOpponent` toward the agent's sensor budget, so the matchup is information-fair; (2) recalibrate the 80% bar against what a 300°-vision agent can physically reach. Phase 2 POCA self-play is *not* indicated — runs `v2`/`v3`/`p3` were effectively symmetric self-play and scored 15–19%.
