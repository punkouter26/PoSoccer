...
SETUP (one time, and it needs a human in a browser):
  ...
  1. Play Console -> Create app -> PoSoccer. The application id below has to be
  ...
  4. Play Console -> Users and permissions -> Invite new user -> paste the
     ...
     and use Play App Signing" (or narrower) for the PoSoccer app.
  ...
  5. Put the JSON next to the keystore, which is already the project's
     convention for secrets that must stay out of the repo:
       C:/Users/punko/Downloads/PoSoccer-Release/play-service-account.json
...
PACKAGE_NAME = "com.punkoutersoftware.posoccer"

DEFAULT_AAB = os.path.join(REPO_ROOT, "Builds", "Android", "PoSoccer.aab")
DEFAULT_CREDENTIALS = "C:/Users/punko/Downloads/PoSoccer-Release/play-service-account.json"
...
def main():
    ...
    parser.add_argument("--aab", default=DEFAULT_AAB,
                        help="path to the .aab (default: Builds/Android/PoSoccer.aab)")
    parser.add_argument("--credentials",
                        default=os.environ.get("POSOCCER_PLAY_CREDENTIALS",
                                               DEFAULT_CREDENTIALS),
    ...
    if not os.path.exists(args.aab):
        fail("no AAB at " + args.aab + " — run PoSoccer → Build Android AAB first")
